...
############################################
# Publication and Country Counts #
...
##################################################################
# Visualisations: Publication and country Counts #
...
#################################################
# Mitigation and Adaptation Outcome Counts  #
...
ggplot(adaptation_counts, aes(x = count, y = term)) + # horizontal bars
  geom_col(fill = "#B891B3", width = 0.7) +
  
  # 2) place labels just to the right of each bar:
  geom_text(
    aes(label = count),
    position = position_nudge(x = 1), # nudge horizontally now
    hjust = 0, # flush-left of the nudged position
    vjust = 0.5, # vertically centered on the bar
    size = 5
  ) +
  scale_x_continuous(
...
ggsave(
  filename = "adaptation_count_plot.png", # or use .pdf or .tiff
  plot = last_plot(), # saves the last ggplot object
  width = 12, # in inches
  height = 8,
  dpi = 600, # high resolution
